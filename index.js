...
function formatCalculation(rub, commission, rubTotal, rate, mntTotal, mntReceived) {
  let calc = '<pre>';
  calc += '+  ' + String(formatNumber(rub, 2)).padStart(13) + '\n';
  calc += '+  ' + String(formatNumber(commission, 2)).padStart(13) + '\n';
  calc += '---------------\n';
  calc += '+  ' + String(formatNumber(rubTotal, 2)).padStart(13) + '\n';
  calc += '*  ' + String(formatNumber(rate, 2)).padStart(13) + '\n';
  calc += '---------------\n';
  calc += '+  ' + String(formatNumber(mntTotal, 2)).padStart(13) + '\n';

  if (mntReceived !== null && mntReceived !== undefined && Number(mntReceived) > 0) {
    calc += '-  ' + String(formatNumber(mntReceived, 2)).padStart(13) + '\n';
    calc += '---------------\n';
    calc += '+  ' + String(formatNumber(mntTotal - mntReceived, 2)).padStart(13) + '\n';
  }
...
function findStateByTxId(chatId, txMessageId) {
  const prefix = String(chatId) + '_';
  for (const [key, state] of transactionStates.entries()) {
    if (String(key).startsWith(prefix) && String(state.txMessageId) === String(txMessageId)) return state;
  }
  return null;
}
  ...
  return lockSheets(async () => {
    ...
    for (const [col, value] of Object.entries(updates)) {
      ...
      if (colIndex !== undefined) {
        requests.push({
          range: SHEET_NAME + '!' + String.fromCharCode(65 + colIndex) + String(rowNum),
          values: [[value]]
        });
      }
    }
...
bot.on('channel_post', async (ctx) => {
  try {
    if (String(ctx.channelPost && ctx.channelPost.chat && ctx.channelPost.chat.id) !== String(CONFIG.RATE_CHANNEL_ID)) return;
    if (!ctx.channelPost || !ctx.channelPost.text) return;

    const text = ctx.channelPost.text;
    const orgMatch = text.match(/🏦[^:]*:\s*([\d.,]+)/);
    const personMatch = text.match(/👤[^:]*:\s*([\d.,]+)/);

    if (orgMatch) cachedRates.org = parseFloat(String(orgMatch[1]).replace(',', '.'));
    if (personMatch) cachedRates.person = parseFloat(String(personMatch[1]).replace(',', '.'));
...
async function processCommission(ctx, state) {
  ...
  if (state.rub >= 10000000) {
    ...
    await ctx.reply(
      '💰 <b>Шимтгэл хэд вэ?</b>\n(Санал: ' + formatNumber(defaultCommission, 2) + ' RUB)',
      { parse_mode: 'HTML' }
    );
  } else {
...
async function showCalculation(ctx, state) {
  ...
  const calc = formatCalculation(state.rub, state.commission, state.rubTotal, state.rate, state.mntTotal, null);
...
async function handleCaptionTransaction(ctx, caption) {
  ...
  if (numberMatch && назначениеMatch && суммаMatch) {
    const stateKey = String(chatId) + '_' + String(messageId);
...
bot.on('photo', async (ctx) => {
  try {
    await handleCaptionTransaction(ctx, (ctx.message && ctx.message.caption) || '');
  } catch (err) {
...
bot.command('report', async (ctx) => {
  ...
  try {
    const raw = String((ctx.message && ctx.message.text) || '').trim();
...
bot.on('text', async (ctx, next) => {
  try {
    ...
    // commands -> next()
    if (String(text).startsWith('/')) return next();
    if (!isUserAllowed(ctx)) return;
    ...
    if (activeState.step === 'waiting_cost_rate') {
      ...
      await ctx.reply('📊 <b>Зарах ханш сонгоно уу:</b>', {
        ...
        ...Markup.inlineKeyboard([
          [
            Markup.button.callback('🏦 ' + Number(rates.org).toFixed(2), 'rate_org_' + activeState.txMessageId),
            Markup.button.callback('👤 ' + Number(rates.person).toFixed(2), 'rate_person_' + activeState.txMessageId)
          ],
    ...
    if (activeState.step === 'waiting_partial_mnt') {
      ...
      await ctx.reply('✅ <b>Хэсэгчлэн орлоо:</b> ' + formatNumber(mnt, 2) + ' MNT\n\n' + calc, {
        parse_mode: 'HTML'
      });
      ...
      if (activeState.mntRemaining <= 0) {
        ...
        if (rowNum) await updateTransaction(rowNum, { completedAt: completedAt, minutes: minutes, status: 'Амжилттай' });

        await ctx.reply('🎉 <b>Гүйлгээ амжилттай хаагдлаа!</b>', { parse_mode: 'HTML' });
        transactionStates.delete(String(chatId) + '_' + String(activeState.txMessageId));
      } else {
...
bot.action(/change_rate_(.+)/, async (ctx) => {
  try {
    ...
    await ctx.editMessageReplyMarkup({
      inline_keyboard: [
        [
          { text: '🏦 ' + Number(rates.org).toFixed(2), callback_data: 'rate_org_' + ctx.match[1] },
          { text: '👤 ' + Number(rates.person).toFixed(2), callback_data: 'rate_person_' + ctx.match[1] }
        ],
...
bot.action(/confirm_full_(.+)/, async (ctx) => {
  try {
    ...
    transactionStates.delete(String(ctx.chat.id) + '_' + String(ctx.match[1]));
  } catch (err) {
...
bot.action(/confirm_partial_(.+)/, async (ctx) => {
  try {
    ...
    await ctx.reply('💸 <b>Ороод ирсэн MNT дүнг оруулна уу:</b>', { parse_mode: 'HTML' });
  } catch (err) {
...
async function start() {
  ...
  if (CONFIG.WEBHOOK_DOMAIN) {
    const webhookUrl = String(CONFIG.WEBHOOK_DOMAIN) + String(webhookPath);
